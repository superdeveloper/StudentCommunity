...
class HighlightView {
    ...
    void handleMotion(int edge, float dx, float dy) {
        ...
        if (edge == MOVE) {
            ...
            // Nevil - allow to decrease the size of crop rect only till CropImageActivity.CROP_SIZE_MIN
            if( cropRect.width() >= CropImageActivity.CROP_SIZE_MIN && cropRect.height() >= CropImageActivity.CROP_SIZE_MIN) {
            ...
            {
                // Nevil - if crop rect size decreases less than CropImageActivity.CROP_SIZE_MIN then increase it by a little amount
                RectF r1 = new RectF(cropRect);
                r1.left = r1.left - 2;
                r1.right = r1.right + 2;
